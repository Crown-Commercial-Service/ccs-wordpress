...
if [ ! -e "$FIRST_RUN_PATH" ]; then
    ...
    if [ "$APPLICATION_NAME" == "$IMPORT_APP_NAME" ]; then
        ...
        sudo curl -O https://releases.deadmanssnitch.com/field-agent/1.0.8/dms_1.0.8_linux_arm64.tar.gz
        sudo tar zxvf dms_1.0.8_linux_arm64.tar.gz -C /usr/local/bin
